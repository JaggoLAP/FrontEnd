...
const cuadroTexto = document.getElementById('inputchat');

cuadroTexto.addEventListener('keyup', function(event) {
  if (event.key === 'Enter') {
    enviarMensaje();
  }
});
...
function cargarServidores() {
    ...
    fetch(url, {
        ...
        .then(data => {
            ...
            contmjes.innerHTML = "";
            const Uside = document.getElementById("Uside");
            Uside.innerHTML = "";
            ...
            data.forEach(servidor => {
                if (Band === 0) {Uside.textContent = servidor.usuario_id; Band += 1;}
...
function cargarMensajesDesdeCanales(Canalid, usuarioId) {
    ...
    .then(data => {
        ...
        data.forEach(mnsje => {
            ...
            contmjes.scrollTop = contmjes.scrollHeight
        });
    });
}

function enviarMensaje() {
    let usuarioId_Obj = document.getElementById("Uside");
    let Canalid_Obj = document.getElementById("Chide");
    let Canalid = Canalid_Obj.textContent;
    let usuarioId = usuarioId_Obj.textContent;
    console.log('Canalid mjes: ',Canalid);
    console.log('usuarioId mjes: ',usuarioId);
    const inputchat = document.getElementById("inputchat");
    let mensaje = inputchat.value.trim();
    ...
    .then(response => {
        if (response.status === 201) {
            ...
            return response.json().then(data => {
                document.getElementById("contmjes").innerHTML = data.message;
            });
        }
    })
    .catch(error => {
        document.
            getElementById("contmjes").innerHTML = "Ocurrió un error.";
    });
